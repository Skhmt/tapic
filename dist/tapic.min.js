...
g.get("color")||g.set("color","#d2691e"),g.set("badges",g.get("badges").split(",")),l=b.slice(4).join(" ").substring(1),m("whisper",{from:g.get("display-name"),to:b[3],color:g.get("color"),emotes:g.get("emotes"),turbo:1==g.get("turbo"),message_id:g.get("message-id"),thread_id:g.get("thread-id"),user_id:g.get("user-id"),text:l,badges:g.get("badges")});else if("CLEARCHAT"===b[1])m("clearChat");else if("CLEARCHAT"===b[2]){for(var l=b,B=l[0].slice(1).split(";"),b="",g=1,t=0;t<B.length;t++){var n=B[t].split("=");
